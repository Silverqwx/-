...
{
	if (_u<kernel_mid_y || _u>_src.rows - kernel_mid_y || _v<kernel_mid_x || _v>_src.rows - kernel_mid_x)
	{
...
float QWX_Optimized_Position_Orientation::QWX_convolution(const Mat &_src, const vector<int> &_rotated_kernel, int _u, int _v)
{
	...
	{
		if (_rotated_kernel[i]<-kernel_mid_y || _rotated_kernel[i]>_src.rows + kernel_mid_y || _rotated_kernel[i + 1]<-kernel_mid_x || _rotated_kernel[i + 1]> _src.rows - kernel_mid_x)
			continue;
